...
tiempoTurno = ''
turno = True
ponerNombre=False  #cuando sea True pone el nombre del J1
proceso=0
proceso2=0
...
def actualizarPuntaje(letraSeleccionada,comodin):
    ...
    """
    multiplicadorP = 1
    print(letraSeleccionada,comodin)
    #puntajeA.get()
    puntaje = 0
    print(puntaje)


    if ponerNombre == True : #Es el puntaje del J1
    ...
    if comodin!= 0: #Si la casilla no es cero, es porque hay un valor que se debe multiplicar
        if comodin == 1:        
            puntaje = puntos*2
...
def tiempoTurno(contador=5,contador1=0):
...
def cambiarTurno():
    ...
    if ponerNombre == True:
        ...
        ponerNombre = False
    elif ponerNombre == False:
        jugador.set('Jugador: '+nombres[1])
        ponerNombre = True

    tiempoTurno(contador=5,contador1=0)
